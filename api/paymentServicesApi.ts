...
export class PaymentServicesApi {
     ...
     */
    public async listPaymentServices (limit?: number, cursor?: string, method?: 'card' | 'paypal' | 'banked', environment?: 'development' | 'staging' | 'production', options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: PaymentServices;  }> {
        const localVarPath = this.basePath + '/payment-services';
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this._defaultHeaders);
        const produces = ['application/json'];
        ...
        if (method !== undefined) {
            localVarQueryParameters['method'] = ObjectSerializer.serialize(method, "'card' | 'paypal' | 'banked'");
        }
