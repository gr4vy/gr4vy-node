...
describe('#authorizeNewTransaction', () => {
  test('it should create a transaction', async () => {
    ...
    const transaction = await client.authorizeNewTransaction(transactionRequest).catch(error => {
    ...
    expect(transactionId.length).toEqual(36)
  })
})
...
describe('#captureTransaction', () => {
  test('it should capture a specific transaction', async () => {
    ...
    const transaction = await client.captureTransaction(transactionId, transactionCaptureRequest);
...
describe('#refundTransaction', () => {
  test('it should refund a specific transaction', async () => {
    ...
    const transaction = await client.refundTransaction(transactionId, transactionRefundRequest);
